...
- **mac80211** — `subsys/400-mac80211-defer-ap-side-ft-key-upload`
  (PR 23181, in sync with the PR head `7010be86f5`): defers AP-side FT key
  ...
  zero offset. **Updated again 2026-09-11** (`ecd3300a61` → `7010be86f5`): the flag refresh below now computes into a local and publishes once with `WRITE_ONCE()`, instead of clearing and re-setting `key->conf.flags` in place on an already-published key where RX/TX could observe the cleared state — a race the previous revision introduced. **Updated 2026-09-06** (`75311aa2c8` → `ecd3300a61`):
...
- **mt76 AP-interface lifetime races** (PR 24810, two patches by Ryan
  Leung, also sent to the kernel) — carried as `9999-23` / `9999-24`.
  Closes upstream issue 24594, a filogic kernel crash + ramoops when
  bringing up 2.4 GHz AP mode, and neither fix is in our mt76 pin.
  `9999-23` moves `idr_destroy(&wcid->pktid)` inside `status_lock` in
  `mt76_wcid_cleanup()`, where it can no longer race `idr_alloc()` from
  `mt76_tx_status_skb_add()`; because idr nodes come from the generic
  slab allocator, that corruption typically resurfaced as an unrelated
  crash elsewhere, which is why the reporter saw random ramoops rather
  than an mt76 backtrace. `9999-24` publishes `dev->mt76.wcid[idx]`
  before the MCU add commands, so firmware TX-status/TX-free events
  cannot arrive at a handler with no published WCID. **`9999-23` is the
  PR's file verbatim, but `9999-24` is pesa1234's variant, not the
  PR's** — identical payload, different anchor: the PR targets
  `mt7915_add_interface`, while this fork carries `1012` (which splits
  `mt7915_init_vif` out of it) and `9999-08` (which moves
...
| `obss_interval` | `300` | `obss_interval=300` emitted only when `ht_coex` is enabled; otherwise not emitted. User-overridable since 2026-09-06 (was hardcoded) |
...
`obss_interval` **is** a user option since 2026-09-06 (it was previously
hardcoded): the generator applies `set_default(config, 'obss_interval', 300)`
and emits it whenever `ht_coex` is enabled on the radio, so the default is
unchanged but a radio can override it. `mu_onoff` was added at the same time
(MU-MIMO control bitmap, 0-15); hostapd's `mtk/0014` already parsed the key
but nothing emitted it, so it was unreachable from UCI. It has no schema
default, so it is emitted only when set.
